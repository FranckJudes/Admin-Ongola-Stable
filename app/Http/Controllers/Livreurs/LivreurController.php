...
class LivreurController extends Controller
{
    ...
    public function go_to_add_order_livreurs(){
        $commandeEnPreparation = suivie_livraison::where('etat', '2')->with('commande')->paginate(50);
        $Livreurs = Livreurs::where('status','1')->get();
        return view('livreurs.commande_livreurs',compact('commandeEnPreparation','Livreurs'));
    }

    public function commandesParDate($livreurId, $startDate = null, $endDate = null)
    {
        try {
            //code...
            $query = Livreurs::findOrFail($livreurId)->commandes();
    ...
    public function peformance_livreurs(){
        try {
            $livreurs = Livreurs::paginate(5);
            return view('livreurs.performance',compact('livreurs'));
        } catch (\Throwable $th) {
            //throw $th;
        }
    }
